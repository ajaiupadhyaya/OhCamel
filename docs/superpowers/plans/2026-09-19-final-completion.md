...
12. **Images and deploys.** CI builds both images on every push and runs the compose harness smoke against them; only a green `main` publishes `ghcr.io/ajaiupadhyaya/ohcamel{,-research}:<sha>` and `:main`. Compose references `${OHCAMEL_TAG}` with no `build:` block; `deploy.sh` deploys a sha by pulling. Its `--build` fallback builds **both** `deploy/Dockerfile` and `deploy/research.Dockerfile` and tags both `ghcr.io/ajaiupadhyaya/ohcamel:SHA` and `ghcr.io/ajaiupadhyaya/ohcamel-research:SHA`, because until owner step O10 settles GHCR visibility the fallback is the only deploy path and a one-image fallback would leave the research container on a missing or stale image. Rollback is `deploy.sh --live --sha <previous>`. A live deploy is refused on a weekday inside 09:25-16:10 America/New York time unless `--during-market` is passed — a fixed UTC band was tried and killed in Task 4's review because it leaves the last 50 minutes of every EST session unguarded once the clock leaves daylight time, and a restart drops the one allowed stream and forces reconciliation.
...
4. **The market-hours guard (ruling 12).** A live deploy is refused on a weekday inside 09:25-16:10 America/New York time unless `--during-market` is given, with a message saying why. `DEPLOY_NOW` overrides the clock for the test.
...
**Tests:** `bash -n deploy/deploy.sh` passes. `deploy/test/deploy_guard_test.sh` prints ok for 14 hand cases (ruling 12), read as America/New York wall-clock time: Monday 09:24 allowed; Monday 09:25 refused; Wednesday 16:09 refused; Wednesday 16:10 allowed; Saturday allowed; a winter afternoon and a winter morning inside the window refused, a winter morning before it allowed; a summer afternoon past the window's DST-shifted close allowed; the spring-forward and fall-back minute pairs allowed on both sides of the transition; and `--during-market` flips an otherwise-refused reading to allowed. The `lint` job is green on the branch and a seeded shellcheck error on a throwaway commit fails it (the run URL goes in the report and the commit is reverted).
...
**What must become true:**
1. **A whole-branch review** of Stage 0, explicitly covering `dae577e`'s fix round: duplicate bars, duplicate DGS10 dates and a negative window are `Error`s rather than exceptions; a duplicate instrument is deduplicated silently (`List.stable_dedup`), not an `Error` — `test_duplicate_instrument_is_deduplicated` pins it; `last_close` and `adv20` read no bar after `as_of`; a stale DGS10 is an `Error`; the factor order is tied to `Factor.all`; and the four surviving mutations are pinned.
...
4. Refuse a live deploy inside the 09:25-16:10 America/New York window unless `--during-market` (Task 4's guard, kept — ruling 12).
...
**O2 — deploy W2, A3 and Stage 0 (after Stage 0 merges; outside the 09:25-16:10 America/New York trading window — ruling 12 — or pass `--during-market` to override that deliberately).** The first `--live` run builds `ohcamel-research` for the first time ever, which may take about 20 minutes if the opam layer changed. `deploy.sh`'s build step and market guard were linted in CI by Stage 0's own `lint` job before this step ran.
```
ssh ohcamel
cd ~/OhCamel
git pull --ff-only
