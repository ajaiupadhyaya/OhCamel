...
> `docs/superpowers/plans/2026-09-19-final-completion.md`, which absorbed
> this phase's remaining work into Stages 2A and 2B. It is kept, unedited
> below, for one reason: it is where two numbers in the shipped kernel were
> argued. Ruling 5's conditioning floor is 0.01, not the 1e-8 first written
> here — 1e-8 accepted betas of ±9,000 from a panel that was collinear
> rather than singular. Ruling 7 decides Cornish–Fisher monotonicity in
> closed form from the quadratic's derivative, not by sampling 801 points —
> the sample missed a narrow violation. Tasks 3, 5 and 7a shipped -- as the
> pure modules `lib/factor_model.ml`, `lib/liquidity.ml` and the
> Cornish-Fisher half of `lib/risk_metrics.ml`, carried onto the finish
> plan's Stage 0 branch and still unwired to anything. No other task here
> shipped, and the wiring their text describes is not what will be built:
> Stages 2A and 2B replaced it. Read the final plan for that. The branches
> this text lived on are tagged `archive/a4-plan-v2`, `archive/a4-t3`,
> `archive/a4-t5` and `archive/a4-t7a`.
