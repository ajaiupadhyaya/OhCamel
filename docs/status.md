...
| Deployed | 2026-09-13, both hosts, from `cf79764` (phase A1: the desk library beside the risk kernel; a SQLite journal on the live host's `desk_data` volume; the live book's quantities and cash read from the Alpaca paper account every minute; a session close that rolls the return windows and records a VaR forecast per estimator) — both origins report it as `build.git_sha` on `/api/ops` — verified by the production smoke suite: 23 passed, 0 failed, 0 skipped. The reports take 1.2 s before listen; the GARCH study 18.4 s on the second domain. On the live host the engine opened `/data/desk.db` (owned by uid 10001, its WAL beside it), read the paper account, and its first sync succeeded; no session close is recorded yet. Previous deploy, and the rollback reference: `eb891eb` (2026-09-12, phase W1: Figure 1) |
...
- **440 hermetic tests**, plus seven scheduler cases in `test/desk_async` —
...
- **Coverage 76.5%** (5,933 / 7,753 instrumented points in `lib/` and `desk/`,
