...
let test_an_order_is_sent_as_strings () =
  (* Six fields in the order order_request_json writes them: the quantity 2 as
     ...
     "day" because a ticket is a day order (spec §3.6); and no
...
(* A rebalance's market-on-open order: the same six fields, with
   time_in_force "opg", the word Alpaca queues for the next opening auction. *)
let test_a_market_on_open_order_is_sent_as_opg () =
  Alcotest.(check string)
    "opg"
    {|{"symbol":"AAPL","qty":"2","side":"buy","type":"market","time_in_force":"opg","client_order_id":"ohc-01M2B0CWJ0ZZZZZZZZZZZZZZZ1"}|}
    (Yojson.Safe.to_string
       (Alpaca.order_request_json
          { (request Order.Kind.Market) with Order.Request.tif = Order.Tif.Opg }))
...
let suite =
  ( "alpaca_trading",
    [
      Alcotest.test_case "a market-on-open order is sent as opg" `Quick
        test_a_market_on_open_order_is_sent_as_opg;
