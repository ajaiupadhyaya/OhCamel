...
let test_one_position () =
  ...
  Alcotest.check feq "spread cost" 500.0 (get_opt "spread cost" line.L.Line.spread_cost);
  (* Totals of a single-line book equal that line's own figures. *)
  Alcotest.check feq "total spread cost" 500.0
    (get_opt "total spread cost" t.L.spread_cost);
  ...
  Alcotest.check feq "LVaR = VaR + spread cost" 50_500.0
    (get_opt "lvar" (L.lvar ~var:50_000.0 t))
...
let test_zero_spread_gives_lvar_equal_var () =
  ...
  Alcotest.(check bool)
    ...
    (Float.equal (get_opt "spread cost" t.L.spread_cost) 0.0);
  Alcotest.(check bool)
    "LVaR equals VaR exactly" true
    (Float.equal (get_opt "lvar" (L.lvar ~var:50_000.0 t)) 50_000.0)
...
let test_lvar_at_least_var () =
  ...
  Alcotest.(check bool)
    ...
    (Float.( >= ) (get_opt "lvar" (L.lvar ~var:50_000.0 t)) 50_000.0)
...
let test_totals_are_sums_of_lines () =
  ...
  let expected_spread =
    List.fold t.L.lines ~init:0.0 ~f:(fun acc l ->
        acc +. get_opt "line spread cost" l.L.Line.spread_cost)
  in
  let expected_impact =
    List.fold t.L.lines ~init:0.0 ~f:(fun acc l ->
  ...
  Alcotest.(check bool)
    "spread cost is positive" true
    (Float.( > ) (get_opt "total spread cost" t.L.spread_cost) 0.0);
  Alcotest.(check bool)
...
(* A nonzero position missing ADV: its own days, impact fraction and impact
   ...
   neither ADV nor sigma, only a finite half_spread_bps) is still known --
   [Some 20.0], not [None]. *)
let test_missing_adv_nulls_the_line_and_the_totals () =
  ...
  Alcotest.check feq "spread cost is still known" 20.0
    (get_opt "spread cost" line.L.Line.spread_cost);
  check_none "total impact cost" t.L.impact_cost;
  check_none "total max days" t.L.max_days_to_liquidate;
  (* And the total spread cost is not poisoned by the missing ADV either --
     only the half-spread guard can do that (see the module header). *)
  Alcotest.check feq "total spread cost is still known" 20.0
    (get_opt "total spread cost" t.L.spread_cost)
...
let test_zero_qty_with_missing_data_is_not_none () =
  ...
  Alcotest.check feq "spread cost is zero (x = 0)" 0.0
    (get_opt "spread cost" line.L.Line.spread_cost);
...
(* ------------------------------------------------------------------------ *)
(* THE HALF-SPREAD'S OWN GUARD (Task 3, "the finish")                       *)
(* ------------------------------------------------------------------------ *)

(* A small, self-contained worked example of spread_cost alone, independent
   of the 10,000-share/$100 example above: 100 shares at $50.00 with a 5 bps
   half-spread.
     |x| = 100 * 50.00 = 5,000
     spread_cost = |x| * half_spread_bps * 1e-4 = 5,000 * 5 * 1e-4
                 = 5,000 * 0.0005 = 2.50 *)
let test_spread_cost_exact () =
  let p =
    {
      L.symbol = sym "SPY";
      qty = 100.0;
  ...
  in
  match L.line ~participation:0.10 ~impact_coefficient:1.0 p with
  | None -> Alcotest.fail "a finite half-spread must not return None"
  | Some line ->
      Alcotest.check feq "spread cost = 100 * 50.00 * 5e-4" 2.50
        (get_opt "spread cost" line.L.Line.spread_cost)

(* half_spread_bps = nan is a second guard, independent of config.ml's
   Desk_spec.validate refusing it at load time (lib/config.ml) -- a graph
   node body may not raise, so this module hands back [None] rather than
   letting nan reach spread_cost untouched. [L.line] itself is [None], and
   because [L.compute] represents that position with [unknown_line] (present,
   so the position is not silently missing from the book, but unknown in
   every field), every one of the book's totals -- spread_cost, impact_cost
   and max_days_to_liquidate, and therefore lvar too -- is [None], even
   though this position's ADV and sigma (inherited from [one_position]) are
   otherwise perfectly good. *)
let test_non_finite_half_spread_is_none () =
  let p = { one_position with L.half_spread_bps = Float.nan } in
  (match L.line ~participation:0.10 ~impact_coefficient:1.0 p with
  | None -> ()
  | Some _ -> Alcotest.fail "a nan half-spread must make the line None");
  let t = L.compute ~participation:0.10 ~impact_coefficient:1.0 [ p ] in
  Alcotest.(check int) "the position still gets a line entry" 1 (List.length t.L.lines);
  let line = List.hd_exn t.L.lines in
  check_none "spread cost" line.L.Line.spread_cost;
  check_none "days" line.L.Line.days_to_liquidate;
  check_none "impact fraction" line.L.Line.impact_fraction;
  check_none "impact cost" line.L.Line.impact_cost;
  check_none "total spread cost" t.L.spread_cost;
  check_none "total impact cost" t.L.impact_cost;
  check_none "total max days" t.L.max_days_to_liquidate;
  check_none "lvar" (L.lvar ~var:50_000.0 t)
...
let suite =
  ( "liquidity",
    [
      ...
      Alcotest.test_case "spread cost, exact: 100 sh at 50.00, 5 bps = 2.50" `Quick
        test_spread_cost_exact;
      Alcotest.test_case "a non-finite half-spread makes the line and the totals None"
        `Quick test_non_finite_half_spread_is_none;
